...
typedef struct {
  ...
  pthread_cond_t  core_cv;  //cond var for processor core modification
  

  thread_state_t  state;

  //attributes below are for multicore simulation
  int cid;   //assigned core id.
  int binded;  //if this thread is binded for particular core, binded = 1, otherwise 0
  struct timespec core_ts;  //the time spec of when this thread is assigned to the current core.
