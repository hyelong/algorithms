...
class Solution{
public:
	vector<int> getIdx(vector<int>& nums){
		vector<int>	res;
		int n = nums.size();
		vector<int> lsum(n+2, 0);
		vector<int> rsum(n+2, 0);
		for(int i=0; i<n; i++){
			lsum[i+1] = lsum[i] + nums[i];
			rsum[n-i] = rsum[n-i+1] + nums[n-1-i];
		}
		for(int i=1; i<=n; i++){
			if(lsum[i]==rsum[i])	res.push_back(i-1);
		}
		return res;
	}
};
...
8. 给一个string,可以add/delete/change，还可以combine几种来使他变为合法的palindrome,问最少需要几步。
比如：ebabc可以把c改成e
...
public:
    int takeMost(vector<int> nums){
        ...
        int n=nums.size();
        vector<int> first_remain = nums[1]>nums[n-1] ? vector<int>(nums.begin()+2, nums.end()) 
                    : vector<int>(nums.begin()+1, nums.end()-1);
        vector<int> last_remain = nums[0]>nums[n-2] ? vector<int>(nums.begin()+1, nums.end()-1) 
                    : vector<int>(nums.begin(), nums.end()-2);
    ...
    int takeMost(vector<int> nums){
        ...
        for(int len=1; len<=nums.size(); len++){
            for(int i=0; i+len-1<=nums.size()-1; i++){
                ...
                dp[j][i] = max(nums[j]+nums[j+1]>nums[i]?dp[j+2][i]:dp[j+1][i-1], 
                                nums[i]+nums[j]>nums[i-1]?dp[j+1][i-1]:dp[j][i-2]);
            }
        }
    }
};
